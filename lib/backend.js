...
var shouldWarn = typeof describe === 'undefined'; // always except for tests
...
function Backend(options) {
  ...
  if (!options.disableDocAction) {
    if (shouldWarn) console.warn('DEPRECATED: "doc" middleware action');
    this._shimDocAction();
  }
  if (!options.disableSpaceDelimitedActions) {
    if (shouldWarn) console.warn('DEPRECATED: "after submit" middleware action');
