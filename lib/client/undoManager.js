...
function getBatch(list, millisecondsGap) {
  var lastIndex = list.length - 1;
  /* istanbul ignore if */
  if (lastIndex < 0) throw new Error('List empty');

  var firstTime = list[lastIndex].timestamp;
  var batchOp = [list[lastIndex]];
  for (var i = lastIndex - 1; i >= 0; i--) {
    if (Math.abs(list[i].timestamp - firstTime) < millisecondsGap) {
      batchOp.push(list[i]);
    };
  }
  return batchOp;
...
UndoManager.prototype.undo = function(options, callback) {
  ...
  var ops = getBatch(this._undoStack, this._composeInterval);
...
UndoManager.prototype.redo = function(options, callback) {
  ...
  var ops = getBatch(this._redoStack, this._composeInterval);
...
UndoManager.prototype.onDocCreate = function (_doc) {
