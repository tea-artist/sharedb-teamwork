...
Doc.prototype._submit = function(op, options, callback) {
  ...
  op.transactionKey = options.transactionKey;
  op.opCount = options.opCount;
  delete options.opCount;
  delete options.transactionKey;
  ...
  try {
    this._pushOp(op, options.source, callback);
...
Doc.prototype._pushOp = function(op, source, callback) {
  ...
  if (!op.transactionKey) {
    // append transactionKey on op, to make sure multiple ops in one tick are sent in the same transaction
    op.transactionKey = this.connection.getTransactionKey();
    var doc = this;
    util.nextTick(function () {
      op.opCount = doc.connection.getOpCount();
    });
  }
...
Doc.prototype.submitOp = function(component, options, callback) {
  ...
  var submitOptions = {
    ...
    transactionKey: options && options.transactionKey,
    opCount: options && options.opCount
  };
...
Doc.prototype.create = function(data, type, options, callback) {
  ...
  var transactionKey = options && options.transactionKey;
  var opCount = options && options.opCount;
  this._submit(op, { source: source, transactionKey: transactionKey, opCount: opCount }, callback);
};
