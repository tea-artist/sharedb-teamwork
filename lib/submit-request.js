...
SubmitRequest.prototype.submit = function(callback) {
  ...
  var transactionKey = this.extra && this.extra.transactionKey;
  var opCount = this.extra && this.extra.opCount;
  var snapshotOptions = {
    agentCustom: request.agent.custom,
  ...
  backend.db.getSnapshot(collection, id, fields, snapshotOptions, function(err, snapshot) {
    ...
    if (op.v == null) {
      if (op.create && snapshot.type && op.src) {
        ...
        // differentiate.
        backend.db.getCommittedOpVersion(collection, id, snapshot, op, getOpOptions, function (err, version) {
          if (err) return callback(err);
          if (version == null) {
    ...
    var from = op.v;
    backend.db.getOpsToSnapshot(collection, id, from, snapshot, Object.assign({ metadata: true }, getOpOptions), function (err, ops) {
      if (err) return callback(err);
...
SubmitRequest.prototype.commit = function(callback) {
  ...
  backend.trigger(backend.MIDDLEWARE_ACTIONS.commit, this.agent, this, function(err) {
    ...
    backend.db.commit(
      ...
      request.op,
      request.snapshot,
      Object.assign({}, request.options, request.extra),
